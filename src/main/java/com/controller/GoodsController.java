    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(goods !=null){
            ...
            BeanUtils.copyProperties( goods , view );//把实体数据重构到view中
                ...
                //级联表
                ShangjiaEntity shangjia = shangjiaService.selectById(goods.getShangjiaId());
                if(shangjia != null){
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            GoodsEntity goodsEntity = new GoodsEntity();
//                            goodsEntity.setShangjiaId(Integer.valueOf(data.get(0)));   //农户 要改的
//                            goodsEntity.setGoodsName(data.get(0));                    //农副产品名称 要改的
//                            goodsEntity.setGoodsTypes(Integer.valueOf(data.get(0)));   //农副产品类型 要改的
//                            goodsEntity.setGoodsPhoto("");//照片
//                            goodsEntity.setGoodsKucunNumber(Integer.valueOf(data.get(0)));   //农副产品库存 要改的
//                            goodsEntity.setCuxiaoTypes(Integer.valueOf(data.get(0)));   //是否促销 要改的
//                            goodsEntity.setGoodsOldMoney(data.get(0));                    //农副产品原价 要改的
