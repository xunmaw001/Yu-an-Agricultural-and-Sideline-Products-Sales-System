...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的农副产品订单 </p>
    </div>
    <!-- 标题 -->
    <div class="modal fade" id="goodsCommentbackModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="goodsCommentbackTitle">评价</h5>
                ...
                <div class="modal-body">
                    ...
                    <textarea id="goodsCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitGoodsCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部农副产品订单</li>
                    <li v-for="item in goodsOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.goodsName}}</td>
                            <td style="width: 80px;">{{item.goodsValue}}</td>
                            <td style="width: 250px;">
                                <img :src="item.goodsPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 60px;">{{item.goodsKucunNumber}}</td>
                            <td style="width: 80px;">{{item.cuxiaoValue}}</td>
                            <td style="width: 100px;color: red">{{item.goodsOldMoney}}</td>
                            <td style="width: 100px;color: red">{{item.goodsNewMoney}}</td>
                            <td style="width: 60px;">{{item.goodsClicknum}}</td>
                            <td style="width: 80px;">{{item.shangxiaValue}}</td>
                            <td style="width: 150px;">{{item.goodsOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.goodsOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.goodsOrderValue}}</td>
                            <td style="width: 80px;">{{item.goodsOrderPaymentValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">


                                <button v-if="item.goodsOrderTypes==2" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退款
                                </button>


                                <button v-if="item.goodsOrderTypes==3" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.goodsOrderTypes==4" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(goodsOrderTypes) {
                this.goodsOrderTypes = goodsOrderTypes;
                // 获取列表数据
                layui.http.request('goodsOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    goodsOrderTypes: (vue.goodsOrderTypes == 0)?null:vue.goodsOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('goodsOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    goodsOrderTypes: (this.goodsOrderTypes == 0)?null:this.goodsOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`goodsOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`goodsOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`goodsOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,commentback(id) {
                $("#id").val(id);//设置订单id
                $('#goodsCommentbackModal').modal('show');//打开模态框
            }
            ,submitGoodsCommentback(){
                var goodsCommentbackContent = $("#goodsCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("goodsOrder/commentback?id="+id+"&commentbackText="+goodsCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function goodsOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=goods_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function goodsOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=goods_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('goodsOrder/page', 'get',{
            page: 1,
            limit: limit,
            goodsOrderTypes: (vue.goodsOrderTypes == 0)?null:vue.goodsOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('goodsOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            goodsOrderType: (vue.goodsOrderTypes == 0)?null:vue.goodsOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
