                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="goodsName" id="goodsName" placeholder="农副产品名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">农副产品类型</label>
                          <div class="layui-input-inline">
                              <select name="goodsTypes" id="goodsTypes" lay-filter="goodsTypes">
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <div class="gallery-demo" @click="jumpCheck('../goods/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.goodsPhoto?item.goodsPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.goodsName}}
                                        <span v-if="item.goodsNewMoney"> - ￥{{item.goodsNewMoney}}</span>
                                        <span v-else> - {{item.goodsValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function goodsTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=goods_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function cuxiaoTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=cuxiao_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#goodsName').val() != null && jquery('#goodsName').val() != "" && jquery('#goodsName').val() != "null" ) {
                param['goodsName'] = jquery('#goodsName').val();
            }
         
            var goodsTypes = $("#goodsTypes").find("option:selected").val();
            if (goodsTypes != null && goodsTypes != "" && goodsTypes != "null") {
                param['goodsTypes'] = goodsTypes;
            }
                                                                                

            // 获取列表数据
            http.request('goods/list', 'get', param, function(res) {
