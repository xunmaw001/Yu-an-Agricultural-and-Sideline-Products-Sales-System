        ...
        <div class="cartBox">
            ...
            <div class="order_content">
                <ul class="order_lists" v-for="(item,index) in dataList" v-bind:key="index">
                    ...
                    <li class="list_con">
                        <div class="list_img"><img :src="item.goodsPhoto"></div>
                        <div class="list_text">{{item.goodsName}}</div>
                    </li>
                    <li class="list_price">
                        <p class="price">{{item.goodsNewMoney}} RMB</p>
                    </li>
                    ...
                    <li class="list_sum">
                        <p class="sum_price">{{item.goodsNewMoney*item.buyNumber}} RMB</p>
                    </li>
                    <li class="list_op">
    ...
    var vue = new Vue({
        ...
        methods: {
			...
			,addCartNum(index) {
				...
				layui.http.request(`goods/info/${item.goodsId}`, 'get', {}, function(res) {
					// 库存限制
					if (res.data.goodsKucunNumber && res.data.goodsKucunNumber > 0 && res.data.goodsKucunNumber  <= item.buyNumber) {
						layui.layer.msg(`农副产品信息库存不足`, {
							time: 2000,
							icon: 5
						});
						return
			...
			// 购买
			buyClick() {
				localStorage.setItem('goodss', JSON.stringify(this.dataList));
				window.location.href = '../goodsOrder/confirm.jsp'
			}
        }
    });
